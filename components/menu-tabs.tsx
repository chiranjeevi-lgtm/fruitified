...
type MenuItem = { name: string; price: string; image?: string; bgColor?: string }

const sections: { id: string; label: string; emoji: string; items: MenuItem[] }[] = [
  {
    id: "juices",
    label: "Fresh Juices",
    emoji: "🍊",
          ...
          <motion.div key={active} variants={pageVariants} initial="hidden" animate="visible" exit="exit">
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: "10px" }}>
                {current.items.map((item: MenuItem) => (
                  <motion.div
                    key={item.name}
                    ...
                    style={{
                      background: item.bgColor ?? "rgba(255,255,255,0.58)",
                      backdropFilter: item.bgColor ? "none" : "blur(14px)",
                      WebkitBackdropFilter: item.bgColor ? "none" : "blur(14px)",
                      border: item.bgColor ? `1px solid ${item.bgColor}` : "1px solid rgba(255,255,255,0.88)",
                      borderRadius: "18px",
                      padding: item.image ? "10px 22px" : "17px 22px",
                      display: "flex",
                  ...
                  >
                    ...
                    {/* Middle: bottle image (if available) */}
                    {item.image && (
                      <img
                        src={item.image}
                        alt={item.name}
